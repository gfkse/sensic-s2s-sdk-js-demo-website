...
var videoPlayerInit = function(
  ...
  const playerInstance = new bitmovin.player.Player(containerElement, playerConfig);
  window.players[playerId] = playerInstance;

  let agent;
  if (live) {
    agent = gfkS2s.getAgent(function livePosition() {
      return new Date().getTime();
    }, containerElement.id);
  } else {
    agent = gfkS2s.getAgent(function onDemandPosition() {
  ...
  function playStreamLiveSdk() {
    agent.playStreamLive(
      ...
      videoOffset,
      videoSource.title,
      { screen: getScreen(), volume: getVolume(), speed: getPlaybackSpeedMultiplier() },
      { playerid: playerId, cliptype: 'live' }
    );
  }
  ...
  function playStreamOnDemandSdk() {
    agent.playStreamOnDemand(
      ...
      videoSource.title,
      { screen: getScreen(), volume: getVolume(), speed: getPlaybackSpeedMultiplier() },
      { playerid: playerId, cliptype: 'Sendung' }
    );
  }
  ...
  let currentPlaybackSpeed = getPlaybackSpeedMultiplier();
  ...
  playerInstance.on(bitmovin.player.PlayerEvent.PlaybackSpeedChanged, function() {
    ...
    currentPlaybackSpeed = getPlaybackSpeedMultiplier();
  });
  ...
  playerInstance.on(bitmovin.player.PlayerEvent.ViewModeChanged, function(viewModeChangedEvent) {
    ...
    if (
      viewModeChangedEvent.from === bitmovin.player.ViewMode.Fullscreen ||
      viewModeChangedEvent.to === bitmovin.player.ViewMode.Fullscreen
    ) {
...
function customPositionPlayerInit(contentId, playerId, containerElement, videoSource, customPositionInput) {
  ...
  window.players[playerId] = playerInstance;
  var agent = gfkS2s.getAgent(function() {
    return 0;
  }, playerId);
  ...
  playerInstance.on(bitmovin.player.PlayerEvent.Play, function() {
    agent.playStreamOnDemand(
      ...
      { screen: getScreen(), volume: getVolume() },
      { playerid: playerId, cliptype: 'Sendung' }
    );
  });
  playerInstance.on(bitmovin.player.PlayerEvent.Paused, function() {
...
var videoSources = {
  video1: {
    title: 'Test video',
    progressive: 'https://production.sensic-demo.gfk.com/videos/testvideo.mp4'
  },
  video2: {
    title: 'Gfk brand video',
    progressive: 'https://production.sensic-demo.gfk.com/videos/gfk_brand_video.mp4'
  },
  video3: {
    title: 'Understanding mobile consumer',
    progressive: 'https://production.sensic-demo.gfk.com/videos/understanding_mobile_consumer.mp4'
  },
  video4: {
    title: 'Life without market research',
    progressive: 'https://production.sensic-demo.gfk.com/videos/life_without_market_research.mp4'
  },
  video5: {
    title: 'Test video',
    progressive: 'https://production.sensic-demo.gfk.com/videos/testvideo.mp4'
